...
func (uq *UserQuery) QuerySources() *SourceQuery {
	query := &SourceQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithSources(opts ...func(*SourceQuery)) *UserQuery {
	query := &SourceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withSources = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withSettings; query != nil {
		...
		for _, n := range neighbors {
			...
			node.Edges.Settings = n
		}
	}

	if query := uq.withSources; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.SourcesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "sources": %v`, err)
		}
		query.Where(source.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "sources" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Sources = append(nodes[i].Edges.Sources, n)
			}
		}
	}
