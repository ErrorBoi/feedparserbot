...
// The SourceQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type SourceQueryRuleFunc func(context.Context, *ent.SourceQuery) error

// EvalQuery return f(ctx, q).
func (f SourceQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.SourceQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.SourceQuery", q)
}

// The SourceMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type SourceMutationRuleFunc func(context.Context, *ent.SourceMutation) error

// EvalMutation calls f(ctx, m).
func (f SourceMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.SourceMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.SourceMutation", m)
}
