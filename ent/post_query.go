...
func (pq *PostQuery) Order(o ...Order) *PostQuery {
	pq.order = append(pq.order, o...)
	return pq
}

// QuerySource chains the current query on the source edge.
func (pq *PostQuery) QuerySource() *SourceQuery {
...
func (pq *PostQuery) OnlyXID(ctx context.Context) int {
...
func (pq *PostQuery) WithSource(opts ...func(*SourceQuery)) *PostQuery {
	query := &SourceQuery{config: pq.config}
...
func (pq *PostQuery) sqlAll(ctx context.Context) ([]*Post, error) {
	...
	if query := pq.withSource; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].source_posts; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "source_posts" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Source = n
			}
		}
	}
...
func (pgb *PostGroupBy) Aggregate(fns ...Aggregate) *PostGroupBy {
